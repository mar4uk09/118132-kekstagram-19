...
var NAMES_VARIETIES = ['Иван', 'Хуан Себастьян', 'Мария', 'Кристоф', 'Виктор', 'Юлия', 'Люпита', 'Вашингтон'];
...
var generateMessage = function (min, max, messageArray) {
  ...
  var messageString = messageArr.reduce(function (sum, current) {
    return sum + ' ' + current;
  });
...
var generateComment = function (messageArray, nameArray) {
  ...
  comment.message = generateMessage(MIN_NUMBER_OF_SENTENCES, MAX_NUMBER_OF_SENTENCES, MESSAGE_VARIETIES);
...
var generatePictureDescription = function (num) {
  ...
  for (var i = 0; i < getRandomInt(MIN_COMMENTS_NUMBER, MAX_COMMENTS_NUMBER); i++) {
    pictureDescription.comments.push(generateComment(MESSAGE_VARIETIES, NAMES_VARIETIES));
  }
